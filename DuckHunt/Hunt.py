...
#local imports
import constants
from HuntWindow import HUNT_WINDOW
...
signDisplayTime = 0
maxSignDisplayTime = int(100 / constants.speed)
correctSignCount = 0
...
def Handle_Bone(b, bone, thickness):
    ...
    huntWindow.Draw_Black_Line(base_xVal_scaled,base_yVal_scaled, \
...
def HandleState3(): # hand is centered and the digit is correct for 10 frames
    global successDisplayTime, programState, numberGoal, scaffoldedItems, totalItems,signDisplayTime,maxSignDisplayTime
    # print(successDisplayTime)

    if successDisplayTime < 10:
        successDisplayTime += 1
        huntWindow.drawScene()
        huntWindow.drawDuck(numberGoal,signDisplayTime,maxSignDisplayTime,dead=True)
        # huntWindow.drawRocks()
        ...
        huntWindow.duckPosition = random.sample(range(constants.numberOfRocks),1)[0]
...
def HandleState2(): # hand is centered, trying to sign
    global k, programState, correctSignCount, testData, numberGoal, \
                failDisplayTime,signDisplayTime,maxSignDisplayTime, timeForSignToCount
    ...
    if (len(frame.hands) == 0):
        ...
        if signDisplayTime < maxSignDisplayTime:
            ...
            huntWindow.drawScene()
            huntWindow.drawDuck(numberGoal,signDisplayTime,maxSignDisplayTime)
            huntWindow.drawRocks()
            Handle_Frame(frame)
...
def HandleState1(): # hand is in the frame but not consistantly centered
    ...
    if (len(frame.hands) == 0):
        ...
        huntWindow.drawCenterHand(direction)
...
def HandleState0(): # hand is not in the frame
    ...
    if (len(frame.hands) > 0):
        ...
        huntWindow.drawHelpfulAnimation()

while True:
    huntWindow.Prepare()
    frame = controller.frame()
    huntWindow.displaySessionVsAll(db)
    huntWindow.displayWarmColdBar(correctSignCount, timeForSignToCount)
